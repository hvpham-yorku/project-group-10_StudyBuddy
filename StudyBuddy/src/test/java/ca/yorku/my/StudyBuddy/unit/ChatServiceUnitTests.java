...
import ca.yorku.my.StudyBuddy.dtos.FileAttachmentDTO;
...
class ChatServiceUnitTests {
	...
	private static class TestableChatService extends ChatService {
		...
		TestableChatService(ChatDAO chatDAO, MessageDAO messageDAO, FriendRequestDAO friendRequestDAO) {
			super(chatDAO, messageDAO, friendRequestDAO, mockAuthService());
		...
		// USED FOR MOCK AUTH
		private static ca.yorku.my.StudyBuddy.services.AuthService mockAuthService() {
			ca.yorku.my.StudyBuddy.services.AuthService mock = org.mockito.Mockito.mock(ca.yorku.my.StudyBuddy.services.AuthService.class);
			try {
				org.mockito.Mockito.when(mock.verifyFrontendToken(org.mockito.Mockito.any())).thenAnswer(invocation -> {
					...
					// For the test, just blindly return whatever ID is written after "Bearer "
